...
<div class="tags index">
    ...
    <table>
        ...
        ?>
            <tr<?= $class ?>>
                <td><?= $tag->id ?></td>
                <td style="width: 400px"><?= h($tag->tag) ?></td>
                <td><?= $this->Time->niceShort($tag->created) ?></td>
                <td><?= $this->Time->niceShort($tag->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link(__('Edit'), ['action' => 'edit', $tag->id]) ?>
                    <?= $this->Html->link(__('Delete'), ['action' => 'delete', $tag->id], ['confirm' => __('Are you sure you want to delete # {0}?', $tag->id)]) ?>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
